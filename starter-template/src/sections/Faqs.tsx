                        ...
                        {faqs.map((faq,index)=>(
                            <div key={index} className="bg-neutral-900 p-6 rounded-3xl border border-white/15 font-medium">
                                <div className="flex items-center justify-between text-lg">
                                    <h3>{faq.question}</h3>
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={twMerge("feather feather-plus text-lime-400 flex-shrink-0",selectedIndex===index && 'rotate-45')}><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>


                                </div>
                                <div className={twMerge("mt-6",selectedIndex!==index && 'hidden')}>
                                    <p className={twMerge("text-white/50 ",)}>{faq.answer}</p>
                                </div>
                            </div>
                        ))}
                    </div>
